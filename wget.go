...
func WgetCli(call []string) (error, int, string) {
	inPipe := os.Stdin
	outPipe := os.Stdout
	errPipe := os.Stderr
	wgetter := new(Wgetter)
	...
	err,code,name := wgetter.Exec(inPipe, outPipe, errPipe)
...
// Perform the wget ...
func (w *Wgetter) Exec(inPipe io.Reader, outPipe io.Writer, errPipe io.Writer) (error, int, string) {
	var filenametemp string
	var filename string
	
	if len(w.links) > 0 {
		for _, link := range w.links {
			err, filenametemp := wgetOne(link, w, outPipe, errPipe)
			...
			for hasMoreInLine {
				...
				if err == nil {
					//line from stdin
					err, filenametemp = wgetOne(strings.TrimSpace(string(line)), w, outPipe, errPipe)
...
func wgetOne(link string, options *Wgetter, outPipe io.Writer, errPipe io.Writer) (error,string) {
	...
	if options.IsContinue {
		if options.OutputFilename == "-" {
			return errors.New("Continue not supported while piping"), filename
		}
		//not specified
		if filename == "" {
